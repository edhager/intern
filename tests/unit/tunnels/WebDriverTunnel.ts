import WebDriverTunnel, {
  WebDriver
} from '../../../src/tunnels/WebDriverTunnel';
...
import {
  ...
  SAFARI,
  WebDriverConfig
} from '../../../src/tunnels/lib/webDriverConfig';
import { createSandbox, SinonSandbox, SinonStub } from 'sinon';
import * as childProcessModule from 'child_process';
import { ExecSyncOptions } from 'child_process';
import * as tunnelChildProcesses from '../../../src/tunnels/lib/tunnelChildProcesses';
import { ChildExecutor } from '../../../src/tunnels/Tunnel';
import { EventEmitter } from 'events';
import { fail } from 'assert';
import { CancellablePromise, Task } from '../../../src/common';
import { join } from '../../../src/core/lib/common/path';

type DownloadFileStub = SinonStub;
...
registerSuite('unit/WebDriverTunnel', {
  beforeEach() {
    ...
    // Stub out execSync.
    execSyncStub = sandbox.stub(childProcessModule, 'execSync');
    // Don't spawn any child processes.
    sandbox.stub(childProcessModule, 'spawn');
  },
          ...
          (_command: string, executor: ChildExecutor): CancellablePromise => {
            return new Task(
              (resolve: () => void, reject: (reason?: any) => void) => {
                executor(mockChildProcess as any, resolve, reject);
              }
            );
          }
        );
      ...
      tests: (function() {
        ...
        BROWSER_NAMES.forEach(browserName => {
          ...
          tests[
            ...
            mockChildProcess.emit('error', new Error('Forced error'));
            return promise;
          };
        });
        return tests;
...
function testDownloadFileStubCalledWith(
  ...
  assert.isTrue(
    ...
    `_downloadFile not called for "${webDriverConfig.name}" as it should have been`
  );
}
